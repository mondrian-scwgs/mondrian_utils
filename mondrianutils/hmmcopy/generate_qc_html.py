...
def get_non_dropout_metrics(non_dropout_df, total_breakdown):
    ...
    if "species" in non_dropout_df:
        human_groupped = non_dropout_df[non_dropout_df["species"] == "grch37"].groupby(
            ["Experimental Condition", "Cell Call"])
        mouse_groupped = non_dropout_df[non_dropout_df["species"] == "mm10"].groupby(
            ["Experimental Condition", "Cell Call"])
        salmon_groupped = non_dropout_df[non_dropout_df["species"] == "salmon"].groupby(
            ["Experimental Condition", "Cell Call"])
...
def generate_html(dataframes, pngs, html_file):
    ...
    for header, df in dataframes:
        ...
        for place_holder in ["place_holder", "line_break_0", "line_break_1", "line_break_2", "line_break_3",
                             "line_break_4"]:
