...
def generate_intervals_cmd(reference, chromosomes, size):
    generate_intervals(reference, chromosomes, size)


@cli.command()
@click.option('--interval', help='specify reference fasta')
@click.option('--num_splits', type=int, help='specify target chromosomes')
def split_interval_cmd(interval, num_splits):
    split_interval(interval, num_splits)


@cli.command()
@click.option('--inputs', multiple=True, help='vcf files to merge')
@click.option('--output', help='merged output vcf')
def merge_vcf_files_cmd(inputs, output):
    merge_vcf_files(inputs, output)
...
@click.option('--chromosomes', multiple=True, default=list(map(str, range(1, 23))) + ['X', 'Y'],
              help='specify target chromosomes')
def genome_size_cmd(reference, chromosomes):
    get_genome_size(reference, chromosomes)


@cli.command()
@click.option('--inputs', multiple=True, help='specify input')
@click.option('--output', help='specify output')
def merge_chromosome_depths_strelka_cmd(inputs, output):
    merge_chromosome_depths_strelka(inputs, output)


@cli.command()
@click.option('--input', help='specify input bam')
def get_sample_id_bam_cmd(input):
    get_sample_id_bam(input)
...
def consensus_cmd(museq_vcf, mutect_vcf, strelka_indel, strelka_snv, consensus_output, counts_output, chromosomes):
    consensus.main(museq_vcf, strelka_snv, mutect_vcf, strelka_indel, consensus_output, counts_output, chromosomes)
...
def vcf_reheader_id_cmd(input, output, tumour, normal, vcf_normal_id, vcf_tumour_id):
    vcf_reheader_id(input, output, tumour, normal, vcf_normal_id, vcf_tumour_id)
...
def update_maf_ids_cmd(input, output, tumour_bam, normal_bam):
    update_maf_ids(input, output, tumour_bam, normal_bam)
...
@click.option('--output', required=True)
def update_maf_counts_cmd(input, counts, output):
    update_maf_counts(input, counts, output)


@cli.command()
@click.option('--infiles', multiple=True, required=True)
@click.option('--output', required=True)
def merge_mafs_cmd(infiles, output):
    merge_mafs(infiles, output)


@cli.command()
@click.option('--inputs', multiple=True, required=True)
@click.option('--output', required=True)
def concat_csv_cmd(inputs, output):
    concatenate_csv(inputs, output)


@cli.command()
@click.option('--input', required=True)
@click.option('--output', required=True)
def fix_museq_vcf_cmd(input, output):
    fix_museq_vcf(input, output)
...
def merge_bams_cmd(inputs, output, threads, tempdir):
    helpers.merge_bams(inputs, output, tempdir, threads)
